...
# main analysis
mode.nb.random.off.diabetes = glmer.nb(Deaths ~ pct_diabetes + factor(q_popdensity)
...
summary(mode.nb.random.off.diabetes)
exp(summary(mode.nb.random.off.diabetes)[10]$coefficients[2,1])
exp(summary(mode.nb.random.off.diabetes)[10]$coefficients[2,1] - 1.96*summary(mode.nb.random.off.diabetes)[10]$coefficients[2,2])
exp(summary(mode.nb.random.off.diabetes)[10]$coefficients[2,1] + 1.96*summary(mode.nb.random.off.diabetes)[10]$coefficients[2,2])
summary(mode.nb.random.off.diabetes)[10]$coefficients[2,4]

# zero inflated
glmmTMB.off.diabetes = glmmTMB(Deaths ~ pct_diabetes + factor(q_popdensity)
                            + scale(log(medhouseholdincome))+scale(education)
                            + scale(date_since_social) + scale(date_since)
                            + (1|state)+ scale(beds/population) 
                            + offset(log(population)), data = aggregated_data, 
...
exp(summary(glmmTMB.off.diabetes)[6]$coefficients$cond[2,1] + 1.96*summary(glmmTMB.off.diabetes)[6]$coefficients$cond[2,2])
summary(glmmTMB.off.diabetes)[6]$coefficients$cond[2,4]

# exclude NY Metro
mode.nb.random.off.nyc = glmer.nb(Deaths ~ pct_diabetes + factor(q_popdensity)
                                  + scale(log(medhouseholdincome))+scale(education)
                                  + scale(date_since_social) + scale(date_since)
                                  + (1|state)+ scale(beds/population) 
