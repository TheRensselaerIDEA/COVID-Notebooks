...
for (state in colnames(GWAS_MRR)) {
   ...
   prt <- c(NA)
    ...
    #view(data)
    title <- paste(state, " Significant variables.", sep = "")
    data <- data %>% 
      mutate(mycolor = ifelse(x>1, "#94c0c6","#c35442"))
    ...
    ggplot(data, aes(x=x, y=y)) +
      geom_point() + 
      geom_segment( aes(x=x, xend=1, y=y, yend=y, color = mycolor), size=1.3, alpha=0.9 ) +
      ylab("Interested Variable") +
      xlab("MRR of interested Variable") +
      ggtitle(title) +
      geom_segment( aes(x=1, xend=1, y=0, yend = length(vals)-1), size = 1.0)
