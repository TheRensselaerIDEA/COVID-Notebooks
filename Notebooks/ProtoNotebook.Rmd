...
```{r echo = F}
  ...
  state_MRs <- array(dim = c(length(names(statesplit)),length(coefs)))
  ...
  for (i in 1:length(names(statesplit))) {
    ...
    if (length(state_MRs[i,]) != length(this_state_MRs)) {
      print(length(state_MRs[i,]))
      print(length(this_state_MRs))
      stop()
    } else {
      state_MRs[i,] = this_state_MRs
    }
  }
  ...
  # After this point, State_MRs holds the values normalized by this:  (this_state_Cs[j]-1.0)/sdev formula.
  
  
  t1 <- as.data.frame(state_MRs, row.names = c(names(statesplit)), col.names = c(as.character(coefs)))
  
  
  t1
  fil <- tempfile('./Notebooks/state_MRs', fileext = ".rds")
  saveRDS(t1, file = fil)
  
  t2 <- as.matrix(state_MRs)
  t2[is.na(t2)] <- 0
  t2[t2 < -1000] <- -1000
  t2[t2 > 1000] <- 1000
  
   
  color_palette <- colorRampPalette(c("blue", "grey", "red"))(n = 500)
  
  heatmap.2(t2, col = color_palette, main = "Effect on MRR per variable per state", ylab = "State", xlab = "factor", trace = 'none', na.color = "black", labRow = names(statesplit), labCol = coefs, offsetRow = 1, margins = c(10,8))
