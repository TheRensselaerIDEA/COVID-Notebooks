...
ggbar.overall <- function(selected.state = "VT", y.value="p_deaths", moving.avg.window=14, remove.title = F) {
  ...
  US.ma <- covid_TS_US_long.cases[c("date", y.value, my_diff)] %>%
    ...
    mutate(ma = c(numeric(moving.avg.window-1), zoo::rollmean(Values, moving.avg.window, align = "right"))) %>%
    filter(ma > 0) %>%
    filter(date > min(state.us.ma$date))
  US.ma$Type <- "US MA"
...
stats.table <- function(selected_state) {
  ...
  stats %>%
    ...
    tab_source_note(
        source_note = md("Data Source: Raifman J, Nocka K, Jones D, Bor J, Lipson S, Jay J, and Chan P. (2020). 'COVID-19 US state policy database.' Available at: [www.tinyurl.com/statepolicies](www.tinyurl.com/statepolicies)")
      ) %>%
