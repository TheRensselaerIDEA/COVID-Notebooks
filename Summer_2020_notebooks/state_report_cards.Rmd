...
ranking <- covid_TS_state_long.cases %>%
  ...
  summarise(
    ...
    cases.mAvg = sum(diff)*100000/(7*max(population)),
    deaths.mAvg = sum(d_diff)*100000/(7*max(population))
    ) %>%
...
ggplot.state <- function(selected.state, y.value="cases", moving.avg.window=7, case.cut.per.county=10, max.labels=10, remove.title = F) {
  ...
  region_palette[selected.state] <- "black"
  ...
  g <- covid_TS_counties.cases.plot %>%
        filter(cases > case.cut.per.county*n_county) %>%
          ggplot(aes_string(
                ...
                scale_color_manual(values=region_palette, aesthetics = c("box.padding")) +
...
shinyApp(
  ui = fluidPage(
    ...
    fluidPage(
      ...
      fluidRow(column(8, style="text-align:center;",
                      ...
                      offset=2)),
      fluidRow(column(12, style="text-align:center;",uiOutput("state.CoT.title"),
                      plotOutput(outputId = "state.CoT"))),
      fluidRow(column(12, style="text-align:center;",uiOutput("state.DoT.title"),
  ...
  server = function(input, output) {
    ...
    output$state.CoT.title <- renderUI({
      state_name <- input$state_name
      tags$h3(paste0(state_name, " COVID-19 Cases Over Time"))
    })
    
    output$state.DoT.title <- renderUI({
      state_name <- input$state_name
      tags$h3(paste0(state_name, " COVID-19 Deaths Over Time"))
    })
    ...
    output$state.CoT <- renderPlot({
      state_name <- input$state_name
      state_initial <- state.abr[state.abr$name == state_name, "abr"]
      ggbar.overall(state_initial, remove.title = T)
    })
    
    output$state.DoT <- renderPlot({
      state_name <- input$state_name
      state_initial <- state.abr[state.abr$name == state_name, "abr"]
      ggbar.overall(state_initial, y.value = "deaths", remove.title = T)
    })
    
    output$state.trends <- renderPlot({
      state_name <- input$state_name
      state_initial <- state.abr[state.abr$name == state_name, "abr"]
      ggplot.state(state_initial, y.value = "diff", remove.title = T)
    })
  }
)
```
