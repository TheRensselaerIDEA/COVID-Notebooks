...
todays.case.data$Diabetes_rate_ldi <- unlist(lapply(todays.case.data$Diabetes_rate, FUN=function(x){-log(pUS.7.diabetes/(x*100))}))
todays.case.data <- todays.case.data %>%
...
ranking <- covid_TS_state_long.cases %>%
  ...
  summarise(
    ...
    cases.mAvg = sum(diff)/(max(population)),
    deaths.mAvg = sum(d_diff)/(max(population))
    ) %>%
  mutate(cases.mAvg.s = as.vector(scale(cases.mAvg))) %>%
  mutate(deaths.mAvg.s = as.vector(scale(deaths.mAvg))) %>%
  mutate(points = 0.5*cases.mAvg.s + 0.5*deaths.mAvg.s) %>%
  arrange(points) %>%
  ...
  #arrange(deaths.mAvg) %>%
  #mutate(DRank = row_number()) %>%
  #mutate(rank = floor((CRank + DRank)/2)) %>%
  #arrange(rank)
...
ggbar.overall <- function(selected.state, y.value="cases", moving.avg.window=14, remove.title = F) {
  ...
  return(state_cases %>%
    ...
    #geom_line(data = US.ma, arrow=arrow(ends = "last"), color="steelblue", show.legend = F) +
    scale_x_datetime(date_breaks = "1 week", date_minor_breaks = "1 day", date_labels = "%b %d") +
...
```{r}
ggbar.overall(selected_state, "p_cases")
```
