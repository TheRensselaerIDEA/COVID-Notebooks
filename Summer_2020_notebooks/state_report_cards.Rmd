...
ggbar.overall <- function(selected.state = "VT", y.value="p_deaths", moving.avg.window=14, remove.title = F) {
  ...
  US.ma <- covid_TS_US_long.cases[c("date", y.value)] %>%
    mutate(US = c(numeric(moving.avg.window-1), zoo::rollmean(eval(parse(text=y.value)), moving.avg.window, align = "right"))) %>%
    filter(US > 0)
  
  state_cases <- state_cases %>%
    left_join(ma, by=c("date" = "date")) %>%
    left_join(US.ma, by=c("date" = "date"))
  ...
  return(state_cases %>%
    ...
    scale_fill_gradient(high = "#ff0000", 
                        ...
                        guide = guide_colorbar(title = paste0("Percentage change in ", category),
                                               ...
                                               barwidth = 15,
                                               frame.colour = "black")) +
    geom_line(aes(x=date, y=ma, linetype="solid"), color="black", arrow=arrow(ends="last"))  + 
    geom_line(aes(x=date, y=US, linetype="solid"), color="steelblue", arrow=arrow(ends="last")) +
    geom_vline(aes(xintercept=state_policy.df[state_policy.df$POSTCODE == selected.state,]$END_STHM, linetype = "dashed"), color="blue") +
    geom_vline(aes(xintercept=state_policy.df[state_policy.df$POSTCODE == selected.state,]$STAYHOME, linetype= "dashed"), color="black") +
...
shinyApp(
  ui = fluidPage(
    ...
    fluidPage(
      ...
      fluidRow(column(8, style="text-align:center;",
                      ...
                      tags$p("Nationwide, states have taken various approaches to mitigate the spread of coronavirus, such as social distancing interventions and encouraging mask use where social distancing is not possible. Studies by the CDC have shown these methods reduced new cases, hospitalizations, and deaths."),
                      tags$b("Data Source: "), tags$a("CDC", href="https://wwwnc.cdc.gov/eid/article/26/8/20-1093_article"),
                      uiOutput("state.report"),
                      offset = 2)),
      fluidRow(column(12, style="text-align:center;",
  ...
  server = function(input, output) {
    ...
    output$state.report <- renderUI({
      state_name <- input$state_name
      state_initial <- state.abr[state.abr$name == state_name, "abr"]
      stats <- covid_TS_state_long.cases %>%
        filter(State == state_initial) %>%
        top_n(n=1, wt=date) %>%
        select(cases, deaths, p_cases, p_deaths)
      policy <- state_policy.df %>%
        filter(POSTCODE == state_initial)
      
      #params <- c("Overall Cases: ", "Cases per 100k: ", "Overall Deaths: ", "Deaths per 100k: ", "Stay At Home Order Start Date: ", "Stay At Home Order End Date: ")
      tagList(
        tags$h2(state_name, " Stats"),
        tags$p(
