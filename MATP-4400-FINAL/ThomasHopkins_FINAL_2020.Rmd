...
diseases$NAME <- NULL
diseases$mortality_rate <- diseases$deaths / diseases$infected
...
diseases$infected <- NULL
diseases$dead <- NULL
covid_features <- diseases['COVID-19',]
covid_features$season_second_peak <- NULL
train_data <- diseases[-c(nrow(diseases)),names(diseases) != 'season_second_peak']
train_labels <- diseases[-c(nrow(diseases)),'season_second_peak']
diseases
```
...
numeric_features <- c('age_range', 'R0', 'mortality_rate')
numeric_train_data <- train_data_pca[,numeric_features]
scaled_train_data <- scale(numeric_train_data)
km <- kmeans(scaled_train_data, 4)
kclass <- as.factor(km$cluster)
heatmap.2(km$centers, 
          dendrogram="row",
          scale="none",
          trace="none",
          main='Scaled Disease Data Cluster Centers',
          cexRow=0.8,
          cexCol=0.8,
          srtRow=0,
          srtCol=0)
...
```{r}
train.df <- data.frame(train_data, class=train_labels)
lda.fit <- lda(class~., train.df)
covid_pred <- predict(lda.fit, covid_features)
barplot(covid_pred$posterior,
        ylab="Probability", 
        xlab="Season of Second Wave", 
        ylim=c(0, 1.0), 
        main="Predicted Season of Second Wave of COVID-19")
```
### Results
