...
Here, <span style='color:#ab2328'><b>red</b></span> indicate non-significance (p > 0.10)<br><br> 
...
```{r echo = F}
  ...
  ifelse(x >0.10,
         cell_spec(x, "html", background = "#ab2328"),  # RED (over .1)
         ifelse(x>0.05,
                cell_spec(x, "html", background ="#d58570"),  # (over .05)
                ifelse(x>0.01,
                       cell_spec(x, "html", background ="#d4e6e8"), # (over .01)
                       cell_spec(x, "html", background ="#94c0c6")))) # (under .01)
